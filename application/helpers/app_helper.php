...
if(!function_exists('db_get_data')) {
	function db_get_data($table_name = null, $where = false, $sort = false) {
		$ci =& get_instance();
		if ($where) {
			$ci->db->where($where);
		}
...
if(!function_exists('display_menu_member')) {
	function display_menu_member($parent, $level) {
		$ci =& get_instance();
		$ci->load->database();
		$ci->load->model('menu/model_menu');
	    $result = $ci->db->query("SELECT a.id, a.label,a.icon_color, a.type, a.link,a.icon, Deriv1.Count FROM `menu` a  LEFT OUTER JOIN (SELECT parent, COUNT(*) AS Count FROM `menu` GROUP BY parent) Deriv1 ON a.id = Deriv1.parent WHERE a.menu_type_id = 4 AND a.parent=" . $parent." and active = 1  order by `sort` ASC")->result();
		   		...
		   		if ($row->type == 'label') {
			        ...
			        if ($row->Count > 0) {
			        	if ($ci->aauth->is_allowed($perms)) {
							...
							$ret .= display_menu_member($row->id, $level + 1);
							$ret .= "</li>";
						}
			        } elseif ($row->Count==0) {
			           if ($ci->aauth->is_allowed($perms)) {
